...
begin
        ...
        controllerSN := StrToInt64(EditSN.Text);         //测试使用的控制器
        comPort := EditSerialPort.Text;            //使用的通信端口

        Text1.Text := '控制器通信' + '-' + IntToStr(controllerSN) + '-' + comPort ;
        ...
        begin
            ...
            strCmd := wudp.CreateBstrCommand(controllerSN,strFuncData );      //生成指令帧 提取记录指令
            strFrame := wserial.serial_comm(strCmd, comPort)           ;      //发送指令, 并获取返回信息
            If ((wserial.ErrCode <> 0) Or (strFrame = '')) Then
                begin
                //没有收到数据,
